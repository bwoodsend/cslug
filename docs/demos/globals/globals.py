...
an_int = ctypes.cast(slug._dll_.an_int, ctypes.POINTER(ctypes.c_int)).contents
assert an_int.value == 42
an_int.value += 5
assert slug._dll_.get_an_int() == an_int.value

a_float = ctypes.cast(slug._dll_.a_float, ctypes.POINTER(ctypes.c_float)).contents
a_double = ctypes.cast(slug._dll_.a_double, ctypes.POINTER(ctypes.c_double)).contents
an_8_bit_int = ctypes.cast(slug._dll_.an_8_bit_int, ctypes.POINTER(ctypes.c_int8)).contents

assert abs(a_float.value - 1 / 3) < 1e-5
assert abs(a_double.value - 1 / 3) < 1e-12
assert an_8_bit_int.value == 43

a_bytes_array = ctypes.cast(slug._dll_.a_bytes_array, ctypes.c_char_p)
a_string = ctypes.cast(slug._dll_.a_string, ctypes.c_wchar_p)
...
contains_null = ctypes.cast(slug._dll_.contains_null, ctypes.c_char_p)

assert contains_null.value == b"This sentence has a "

# Extract the length (just a regular int).
length = ctypes.cast(slug._dll_.len_contains_null, ctypes.POINTER(ctypes.c_int)).contents.value

# Get a raw void (untyped) pointer to our string.
address = ctypes.cast(slug._dll_.contains_null, ctypes.c_void_p).value
